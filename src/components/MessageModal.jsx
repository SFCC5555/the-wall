...
const MessageModal = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch('https://the-wall-ua0c.onrender.com/api/v1/messages', options);
        ...
        <form className="flex flex-col gap-3">
          <div className="flex gap-3">
            <section className="flex flex-col gap-2 w-1/2">
              <label htmlFor="message" className="opacity-75 font-medium italic">
                Message:
              </label>
              <textarea
                value={messageValue}
                ...
                className={`p-3 italic font-medium ${font}-font ${color}-color placeholder:font-sans h-24 min-h max-h-96 border-2 ${
                  error && "border-red-500 focus:outline-red-500 hover:border-red-500"
                } hover:border-black rounded-sm`}
                placeholder="Write a message..."
              />
              {error && <p className="text-red-500 text-sm italic">{"Can't be blank"}</p>}
            ...
            <section className="w-1/2 flex gap-2">
              ...
              <div className="w-1/2 flex flex-col gap-2">
                <label htmlFor="color" className="opacity-75 font-medium italic">
                  Color:
                </label>
                <div>
                  <button
                  ...
                  >
                    {color[0].toUpperCase()}{color.slice(1)} <i className="bi bi-pencil-fill" />
                  </button>
                  {colorMenuStatus && (
                    <section className="border-2 border-gray-200 p-1 pb-2 h-56 overflow-y-scroll">
                      {colorList.map((c) => (
                        ...
                        >
                          {c[0].toUpperCase()}{c.slice(1)}<i className={`bi bi-pencil-fill`} />
                        </div>
                      ))}
                    </section>
                  )}
          ...
          {confirmStatus && (
            <section className="absolute top-0 left-0 preview w-full h-full flex items-center justify-center bg-black overflow-auto">
              <div className={`absolute top-16 ${font}-font ${color}-color w-fit max-w-1/2 px-10 sm:px-20 pb-20 font-medium italic flex flex-col`}>
          ...
          {confirmStatus && (
            <>
              ...
              <p className="absolute bottom-3 w-2/3 text-xs font-medium italic opacity-80">
                This message will be permanently posted on THE WALL and cannot be deleted.
              </p>
            </>
          )}
        </form>
        <button
          onClick={confirmStatus ? () => setConfirmStatus(false) : () => dispatch(changeMessageStatus(false))}
          className={`absolute top-3 ${confirmStatus ? 'bg-black text-white p-2 rounded-sm bi bi-arrow-left right-3 sm:right-5' : 'bi bi-x-circle-fill text-3xl right-3'
